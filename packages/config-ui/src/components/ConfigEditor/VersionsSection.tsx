    ...
    <Card>
      ...
      <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-2">
          <Label htmlFor="version-start-number">Start Version Number</Label>
          <Input
            id="version-start-number"
            type="number"
            min="1"
            value={versions.startNumber ?? 1}
            onChange={(e) => updateVersions({ startNumber: parseInt(e.target.value) || 1 })}
          />
          <p className="text-xs text-muted-foreground">First version number to generate</p>
        </div>

        <div className="space-y-2">
          <Label htmlFor="version-count">Version Count</Label>
          <Input
            id="version-count"
            type="number"
            min="0"
            value={versions.count ?? 3}
            onChange={(e) => updateVersions({ count: parseInt(e.target.value) || 0 })}
          />
          <p className="text-xs text-muted-foreground">Number of versions to generate</p>
        </div>

        <div className="space-y-2 md:col-span-2">
          <Label htmlFor="version-assign-prob">Version Assignment Probability</Label>
          <Slider
            ...
            onChange={(e) => updateVersions({ assignProbability: parseFloat(e.target.value) })}
